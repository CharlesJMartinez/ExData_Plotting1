## Coursera: Exploratory Data Analysis: Week 1 Project: Plot4
...
par(mfrow = c(2,2), oma=c(0,0,0,0), mar=c(4,4,0.5,0.5))

with (plotdata, {
    plot(Time, Global_active_power, type = "l", ylab = "Global Active Power", xlab="")
    plot(Time, Voltage, type = "l", ylab = "Global Active Power", xlab="")
    plot(Time, Sub_metering_1, type="o", col="black", pch=NA, lty=1, xlab="", ylab="Energy sub metering")
    ...
    legend("topright", pch=NA, col=c("black", "blue", "red"), legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
           lty=1, lwd = 2,, bty="n", cex=0.8, xjust=1, y.intersp=1, pt.cex=1, seg.len=3)
    plot(Time, Global_reactive_power, type = "h",xlab="")
})
